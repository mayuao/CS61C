...
#pragma omp parallel
  {
    for (int i = omp_get_thread_num(); i < ARRAY_SIZE;
...
#pragma omp parallel
  {
    int chunk = ARRAY_SIZE / omp_get_num_threads();
    for (int i = omp_get_thread_num() * chunk;
         i < (omp_get_num_threads() == 1 + omp_get_thread_num()
                  ? ARRAY_SIZE
                  : (omp_get_thread_num() + 1) * chunk);
         ++i) {
      z[i] = x[i] + y[i];
    }
  }
